...
public class Shop {
  ...
  public Shop(OfflinePlayer owner, ItemStack product, ItemStack currency, Chest chest, Sign sign) {
    this.owner = owner;
  ...
  /**
   * Purchases an item from the Shop. Returns a boolean whether the transaction has been successful
   * or not.
   */
  public boolean purchaseItem(Player player) {
    ...
    if (chestInventory.contains(product.getType(), getProductQuantity())
        && playerInventory.contains(currency.getType(), getCurrencyQuantity())) {
